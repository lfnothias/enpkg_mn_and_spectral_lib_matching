...
def generate_mn(spectra_query, mn_graphml_output_path, mn_ci_output_path, mn_msms_mz_tol, mn_score_cutoff, mn_top_n, mn_max_links):
    ...
    Args:
        ...
        mn_graphml_output_path (str): Path to export the .graphml MN file
        mn_ci_output_path (str): Path to export the .tsv MN metadata file
    ...
    os.makedirs(os.path.dirname(mn_graphml_output_path), exist_ok=True)
    ms_network.export_to_graphml(mn_graphml_output_path)

    components = sorted_connected_component_subgraphs(ms_network.graph)
    comp_dict = {idx: comp.nodes() for idx, comp in enumerate(components, start=1)}
    ...
    os.makedirs(os.path.dirname(mn_ci_output_path), exist_ok=True)
    comp.to_csv(mn_ci_output_path, sep='\t', index=False)
